...
static int flash_mchp_erase(const struct device *dev, off_t offset, size_t no_of_bytes)
{
	...
	/* If no_of_bytes is 0, just erase one block mentioned in the offset */
	no_of_bytes = (no_of_bytes == 0) ? config->erase_block_size : no_of_bytes;
	int ret = validate_flash_parameters(config, offset, no_of_bytes, OPERATION_ERASE);
