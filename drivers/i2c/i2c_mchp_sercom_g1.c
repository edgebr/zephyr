...
struct i2c_mchp_dev_data {
	...
	uint8_t rx_tx_data;
	bool first_read_after_addr_match;
				     ...
				     uint16_t target_status)
{
	...
	data->first_read_after_addr_match = true;
				  ...
				  uint16_t target_status)
{
	...
	if (((target_status & SERCOM_I2CS_STATUS_DIR_Msk) == SERCOM_I2CS_STATUS_DIR_Msk)) {
		if ((data->first_read_after_addr_match == true) ||
			...
			data->first_read_after_addr_match = false;
...
static int i2c_poll_out(const struct device *dev)
{
	...
	/* Wait for address byte transfer completion */
