...
/*
 * Arbitrary reload count for the RX DMA data channel; it is retriggered by
 * hardware (see pio_uart_rx_dma_init()) long before it could ever run out, so
 * this value only affects how often that hardware handshake happens.
 */
...
/*
 * Constant source word DMA'd into the data channel's reload alias by the
 * control channel below. The value itself does not matter beyond being a
 * sensible reload count; only its address is used as a DMA source.
 */
				...
				uint8_t *rx_dma_buf)
{
	...
	channel_config_set_ring(&dma_cfg, true, __builtin_ctz(CONFIG_UART_RPI_PICO_PIO_RX_RING_BUF_SIZE));
	...
	/*
	 * The control channel is only triggered by the data channel's
	 * CHAIN_TO on completion, at which point the data channel is
	 * guaranteed idle. It then does a single 32-bit write into the data
	 * channel's TRANS_COUNT trigger alias, which reloads its count and
	 * retriggers it purely in hardware, with the data channel's own
	 * read/write address registers left untouched (so the ring position
	 * carries over exactly where it left off).
	 */
	...
	dma_channel_configure(ctrl_chan, &ctrl_cfg,
			      (volatile void *)&dma_channel_hw_addr(data_chan)->al1_transfer_count_trig,
...
static int pio_uart_irq_update(const struct device *dev)
{
	...
	/*
	 * There is no latched interrupt status to snapshot: readiness is
	 * derived live from the DMA ring position / PIO FIFO state on every
	 * check, so this is always considered up to date.
	 */
	return 1;
}
...
static int pio_uart_init(const struct device *dev)
{
	...
	/*
	 * Applied last: real traffic must not reach the RX pin before the SM
	 * is enabled and (in interrupt-driven builds) DMA is already primed
	 * to drain the hardware FIFO, or bytes arriving in that window would
	 * be lost.
	 */
