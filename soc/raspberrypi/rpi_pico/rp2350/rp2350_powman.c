...
/* Bits of POWMAN last_swcore_pwrup identifying the wakeup source. */
#define RP2350_PWRUP_GPIO0_BIT (1u << 1)
#define RP2350_PWRUP_ALARM_BIT (1u << 6)

/* LPOSC nominal frequency, used only as a fallback if measurement fails. */
#define RP2350_LPOSC_NOMINAL_HZ 32768u

/* Real LPOSC frequency (Hz), measured once and cached. 0 = not yet measured. */
...
void rp2350_powman_timer_init(void)
{
	...
	/* Selecting the tick source stops/starts the timer but preserves its count;
	 * only do it while the timer is not already using LPOSC, so repeated entries
	 * do not disturb a running timer.
	 */
...
bool rp2350_powman_arm_gpio_wakeup(uint32_t gpio, bool wake_on_high)
{
	...
	/* Edge-detect (not level): a line resting at the wake level in level mode
	 * keeps current_pwrup_req asserted and the SoC refuses the power-down.
	 */
