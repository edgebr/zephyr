...
{
	zassert_equal(pins, BIT(TEST_PIN), "Detected interrupt on an invalid pin");
	cb_count++;
}

static void callback_level(const struct device *port, struct gpio_callback *cb,
			   gpio_port_pins_t pins)
{
	int ret;
	...
	if (cb_count % cb_count_target == 0) {
		...
		zassert_equal(ret, 0, "Failed to disable pin interrupt in the callback");
	}
}

static void pin_set_and_verify(const struct device *port, unsigned int pin, int val, int idx)
{
...
void test_gpio_pin_interrupt_edge(unsigned int cfg_flags, unsigned int int_flags)
{
	...
	if (ret == -ENOTSUP) {
		TC_PRINT("pin in mode is not supported.\n");
		ztest_test_skip();
		return;
	}
	ret = gpio_pin_configure(port, TEST_OUT_PIN, GPIO_OUTPUT);
	if (ret == -ENOTSUP) {
		TC_PRINT("pin out mode is not supported.\n");
		ztest_test_skip();
		return;
	}
	...
	ret = gpio_pin_configure(port, TEST_OUT_PIN, GPIO_OUTPUT | cfg_out_flag | cfg_flags);
	zassert_equal(ret, 0, "Failed to configure the pin");
	...
	for (int i = 0; i < 6; i++) {
		pin_set_and_verify(port, TEST_OUT_PIN, 1, i);
		if (int_flags & GPIO_INT_HIGH_1) {
			cb_count_expected++;
		}
		zassert_equal(cb_count, cb_count_expected,
			      "Test point %d: Pin interrupt triggered invalid "
			      "number of times on rising/to active edge",
			      i);

		pin_set_and_verify(port, TEST_OUT_PIN, 0, i);
		if (int_flags & GPIO_INT_LOW_0) {
			cb_count_expected++;
		}
		zassert_equal(cb_count, cb_count_expected,
			      "Test point %d: Pin interrupt triggered invalid "
			      "number of times on falling/to inactive edge",
			      i);
	}
	...
	for (int i = 0; i < 6; i++) {
		pin_set_and_verify(port, TEST_OUT_PIN, 1, i);
		pin_set_and_verify(port, TEST_OUT_PIN, 0, i);
		zassert_equal(cb_count, cb_count_expected, "Pin interrupt triggered when disabled");
	}
}

void test_gpio_pin_interrupt_level(unsigned int cfg_flags, unsigned int int_flags,
				   unsigned int interrupt_calls)
{
	...
	ret = gpio_pin_configure(port, TEST_PIN, GPIO_INPUT);
	if (ret == -ENOTSUP) {
		TC_PRINT("pin input mode is not supported.\n");
		ztest_test_skip();
		return;
	}
	zassert_equal(ret, 0, "Failed to configure the pin");
	...
	if ((cfg_flags & GPIO_ACTIVE_LOW) != 0) {
		cfg_out_flag = (pin_out_val != 0) ? GPIO_OUTPUT_LOW : GPIO_OUTPUT_HIGH;
	} else {
		cfg_out_flag = (pin_out_val != 0) ? GPIO_OUTPUT_HIGH : GPIO_OUTPUT_LOW;
	}
	...
	zassert_equal(cb_count, cb_count_expected, "Pin interrupt triggered on level %d",
		      pin_out_val);

	for (int i = 0; i < 6; i++) {
		pin_out_val = (pin_out_val != 0) ? 0 : 1;
		pin_set_and_verify(port, TEST_OUT_PIN, pin_out_val, i);
		cb_count_expected += interrupt_calls;
		zassert_equal(cb_count, cb_count_expected,
			      "Test point %d: Pin interrupt triggered invalid "
			      "number of times on level %d",
			      i, pin_out_val);
		...
		ret = gpio_pin_interrupt_configure(port, TEST_PIN, int_flags);
		zassert_equal(ret, 0, "Failed to re-enable pin level interrupt");
	}
...
ZTEST_SUITE(gpio_mchp_int, NULL, NULL, NULL, NULL, NULL);
