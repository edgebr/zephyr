...
ZTEST(spi_mchp_error_cases, test_SPI_HALF_DUPLEX_not_supported)
{
	int rv;
	int slave_rv;
...
ZTEST(spi_mchp_error_cases, test_SPI_SLAVE_LOOP_not_supported)
{
	int slave_rv;
	struct spi_config spis_config_invalid = spis_config;
...
ZTEST(spi_mchp_error_cases, test_only_SPI_LINES_SINGLE_supported)
{
	int rv;
	int slave_rv;
	struct spi_dt_spec spim_invalid = spim;
...
ZTEST(spi_mchp_error_cases, test_only_8BIT_supported)
{
	int rv;
	int slave_rv;
	struct spi_dt_spec spim_invalid = spim;
...
ZTEST(spi_mchp_error_cases, test_unsupported_frequency)
{
	int rv;
	struct spi_dt_spec spim_invalid = spim;

	spim_invalid.config.frequency = 250000000;

	rv = spi_transceive_dt(&spim_invalid, tdata.stx_set, tdata.srx_set);
	zassert_equal(rv, -ENOTSUP, "Got %d instead", rv);
}

ZTEST(spi_mchp_error_cases, test_spis_tx_buf_too_big)
{
	int slave_rv;
...
ZTEST(spi_mchp_error_cases, test_spis_rx_buf_too_big)
{
	int slave_rv;
...
ZTEST_SUITE(spi_mchp_error_cases, NULL, suite_setup, before, NULL, NULL);
